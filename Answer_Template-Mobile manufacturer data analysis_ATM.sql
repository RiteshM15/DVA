...
--Q1--BEGIN 
SELECT State
FROM FACT_TRANSACTIONS as T INNER JOIN DIM_LOCATION as L 
ON T.IDLocation = L.IDLocation INNER JOIN DIM_MODEL as M 
...
--Q5--BEGIN
SELECT Model_Name,AVG(Unit_price) AS Avg_Price FROM DIM_MODEL AS M
INNER JOIN DIM_MANUFACTURER AS MFR ON M.IDManufacturer = MFR.IDManufacturer
...
SELECT Customer_name,AVG(TotalPrice) as Avg_amt_Spent from DIM_CUSTOMER as A
...
GROUP BY Model_Name
HAVING Count(DISTINCT CASE WHEN D.YEAR IN (2008,2009,2010) THEN D.YEAR END)=3
ORDER BY Model_Name;
...
SELECT Manufacturer_name, SUM(Quantity) as Sales
FROM DIM_MANUFACTURER AS MFR
INNER JOIN DIM_MODEL AS M ON MFR.IDManufacturer = M.IDManufacturer
...
ORDER BY Avg_Spend DESC
 ...
 SELECT TC.IDCustomer, TC.Sales_year, TC.Avg_Spend, TC.Avg_quantity, 
 ((TC.Avg_Spend - lag(TC.Avg_Spend)
 OVER (PARTITION BY TC.IDCustomer ORDER BY TC.Sales_year)) / lag(TC.Avg_Spend)
 OVER (PARTITION BY TC.IDCustomer ORDER BY TC.Sales_year)) * 100 AS Spend_change_percentage
 FROM TopCustomers as TC
 ORDER BY TC.IDCustomer, TC.Sales_year;
